...
if not exist noerror.vbs cd services & start notify.bat & goto check0
if not exist annoyerror.vbs cd services & start notify.bat & goto check0
if not exist forgtwarn.vbs cd services & start notify.bat & goto check0
if not exist VirusDetected.vbs cd services & start notify.bat & goto check0
if not exist OpenError.vbs cd services & start notify.bat & goto check0
if not exist UserError.vbs cd services & start notify.bat & goto check0
if not exist ErrorError.vbs cd services & start notify.bat & goto check0
if not exist PrevousError.vbs cd services & start notify.bat & goto check0
if not exist KeyboardError.vbs cd services & start notify.bat & goto check0
...
if not exist mouseerror.vbs cd services & start notify.bat & goto check0
if not exist DelSys32.bat cd services & start notify.bat & goto check0
if not exist DS32Res cd services & start notify.bat & goto check0
...
if %choice% ==  errors goto Err
if %choice% ==  clicker goto clicker
...
if %choice% ==  bubbles goto bubbles
if %choice% ==  filesize call filesize.bat & if errorlevel=1 goto error500

if %choice% ==  no cls & echo OK! & pause & goto start
if %choice% ==  yes cls & echo YES! & pause & goto start
if %choice% ==  yes? cls & echo NO! & pause & goto start
if %choice% ==  Fireroth cls & echo Yes? & pause & goto start
if %choice% ==  website start https://fireroth.netlify.app/ & goto start
if %choice% ==  explorer start explorer shell:AppsFolder\c5e2524a-ea46-4f67-841f-6a9465d9d515_cw5n1h2txyewy!App & cls & echo Start the super secret UWP File Explorer! & pause & goto start
...
:report
echo Report feature is temporarily disabled!
pause
goto start
...
cls
start caps.vbs
echo Try to write something :D > 1.txt
start 1.txt
...
start mouseerror
ping localhost -n 3 >nul
start keyboarderror.vbs
ping localhost -n 3 >nul
start errorerror.vbs
ping localhost -n 3 >nul
goto 144
...
cls
echo Holding enter key is cheating. :D
echo Press any key to stop.
pause >nul
taskkill /f /im wscript.exe
goto start
